...
pub fn calculate_next_block_base_fee (
    ...
    let new_base_fee = {
        if gas_used > target_gas_used {
            base_fee_peer_gas + ((base_fee_peer_gas * (gas_used - target_gas_used)) / target_gas_used)
                /U256::from(8u64)
        } else {
            base_fee_peer_gas 
                - ((base_fee_peer_gas * (target_gas_used - gas_used)) / target_gas_used)
                / U256::from(8u64)
        } 
    };
...
pub fn crate_new_wallet() -> (LocalWallet, H160) {
